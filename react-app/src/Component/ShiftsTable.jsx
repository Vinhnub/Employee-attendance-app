...
export function ShiftsTable({
  ...
  function expandShift(shift) {
    setExpandedShift && setExpandedShift(shift.id === expandedShift ? null : shift.id);
  }
            ...
            <React.Fragment key={shift.id}>
              ...
              >
                ...
                <td className={`${styles.shiftCell} ${styles.timeCell}`}>
                  {isTodayShifts ? (
                    <div className={styles.timeDisplay}>{formatTime(shift.start_time)}</div>
                  ) : (
                ...
                <td className={`${styles.shiftCell} ${styles.timeCell}`}>
                  {isTodayShifts ? (
                    <div className={styles.timeDisplay}>{formatTime(shift.end_time)}</div>
                  ) : (
                    shift.end_time || 'Không có'
                  )}
                </td>
              </tr>
              {((showEmployeeName && isTodayShifts) || (!showEmployeeName && !isTodayShifts && expandedShift === shift.id)) && (
                <tr className={`${styles.shiftRow} ${getStatusColor(shift)} ${expandedShift === shift.id ? styles.expanded : ""}`} onClick={() => expandShift(shift)}>
                  <td colSpan={showEmployeeName ? 3 : 2} className={`${styles.shiftCell} ${styles.noteCell}`}>
              ...
              {((isTodayShifts && expandedShift === shift.id) || (allowExpand && expandedShift === shift.id)) && (
                <tr className={styles.detailRow}>
                  <td colSpan={showEmployeeName ? 3 : 2} className={styles.detailCell}>
                    <div className={styles.detailContent}>
                      ...
                      {user && user.role === "manager" && isTodayShifts && (
                              ...
                              <UpdateShift
                                ...
                                setPopup={(msg) =>
                                  popup(
                                    <div
                                      style={{
                                        color: msg.includes("success") ? "#28a745" : "#dc3545",
                                        fontWeight: "500",
                                      }}
                                    >
                                      {msg}
